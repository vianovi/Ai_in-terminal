...
def handle(argv, cfg) -> Result:
    # toolkit fetch <url> [--name X] [--to PATH] [--force]
    ...
    if not filtered:
        return Result.fail("Usage: toolkit fetch <url> [--name X] [--to PATH] [--force]", exit_code=2, render=render)

    url = filtered[0]
    out_dir = to_dir if to_dir else (DEFAULT_ROOT / SUBDIR["fetch"])
    ...
    return Result.fail("Fetch failed.", exit_code=4, data=data, render=render)
