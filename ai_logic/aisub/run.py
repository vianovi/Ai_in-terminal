...
# ==========================================================
# 5) SESSION CONTROL: KILL_ALL & CLOSED_APP (SOFT CLOSE)
# ==========================================================

def _extract_proc_targets_from_profile(profile: dict[str, Any]) -> list[str]:
    """Ambil target proses dari profile vars: semua key yang berakhiran *_proc (case-insensitive)."""
    vars_ = profile.get("vars", {})
    ...
    for k, v in vars_.items():
        if not isinstance(k, str) or not k.lower().endswith("_proc"):
            continue
        ...
        if not name:
            continue
        low = name.lower()
        # filter obvious utilities
        if low in ("bash", "sh", "zsh", "fish", "pgrep", "pkill", "setsid", "sleep"):
            continue
        if name not in out:
            out.append(name)
    return out
...
def _proc_running_exact(name: str) -> bool:
    """Check process by exact name using pgrep -x."""
...
def _soft_term_exact(name: str) -> tuple[bool, str]:
    """Soft terminate using pkill -TERM -x <name>."""
    if not shutil.which("pkill"):
        return False, "pkill_not_found"
    try:
        p = subprocess.run(
            ["pkill", "-TERM", "-x", name],
            stdout=subprocess.DEVNULL,
        ...
        # pkill: 0 = ada match dan signal terkirim, 1 = no processes matched
        if p.returncode == 0:
            return True, "sent_SIGTERM"
        if p.returncode == 1:
            return True, "not_running"
        return False, f"pkill_rc={p.returncode}"
    except Exception as e:
        return False, f"{type(e).__name__}: {e}"


def _pick_default_profile_for_close(data: dict[str, Any], target_name: str) -> Optional[dict[str, Any]]:
    profiles = data.get("profiles", [])
    if target_name:
        return _find_profile(data, target_name)
    # default: daily_start kalau ada, else profile pertama
    p = _find_profile(data, "daily_start")
    if p:
        return p
...
def cmd_kill_all(data: dict[str, Any], argv: list[str]) -> int:
    """
    Usage:
      ai run kill_all [profile_name] [--yes]

    Soft-close (SIGTERM) tanpa wait loop. Tidak pakai SIGKILL.
    """
    force_yes = "--yes" in argv

    target_name = ""
    for a in argv[1:]:
        if a and not a.startswith("--"):
            target_name = str(a).strip()
            break

    profile_obj = _pick_default_profile_for_close(data, target_name)
    if not isinstance(profile_obj, dict):
        ansi.print_brief_error("Profil target invalid / tidak ditemukan.")
        return 2

    targets = _extract_proc_targets_from_profile(profile_obj)

    ansi.print_system("KILL_ALL (SOFT CLOSE)")
    print(f"{ansi.c_dim()}Profile:{ansi.c_reset()} {ansi.c_cyan()}{profile_obj.get('name','')}{ansi.c_reset()}")

    if not targets:
        print("Tidak ada target process dari vars (*_proc).")
        return 0

    if not shutil.which("pkill") or not shutil.which("pgrep"):
        ansi.print_brief_error("Butuh 'pkill' dan 'pgrep' (procps-ng).")
        print("Fedora: sudo dnf install -y procps-ng")
        return 2

    print("Target apps (SIGTERM):")
    for t in targets:
        state = f"{ansi.c_green()}RUNNING{ansi.c_reset()}" if _proc_running_exact(t) else f"{ansi.c_dim()}not running{ansi.c_reset()}"
        print(f"  - {ansi.c_cyan()}{t}{ansi.c_reset()} ({state})")

    if not force_yes:
        ok = _confirm(f"\n{ansi.c_yellow()}Lanjut soft-close semua target di atas? [y/N] {ansi.c_reset()}")
        if not ok:
            print("Dibatalkan.")
            return 0

    for t in targets:
        ok, note = _soft_term_exact(t)
        if ok:
            print(f"{ansi.c_green()}✓{ansi.c_reset()} {t} ({note})")
        else:
            print(f"{ansi.c_red()}✗{ansi.c_reset()} {t} ({note})")

    still = [t for t in targets if _proc_running_exact(t)]
    if still:
        print(f"\n{ansi.tag('WARN', ansi.c_yellow())} Masih berjalan: {', '.join(still)}")
        print("Catatan: tidak pakai SIGKILL. Biasanya app nahan karena ada dialog/unsaved.")
        return 1

    ansi.print_system("✓ Semua target sudah diminta exit (soft).")
    return 0


def cmd_closed_app(data: dict[str, Any], argv: list[str]) -> int:
    """
    Usage:
      ai run closed_app [profile_name] [--wait=20] [--yes]

    Soft-close (SIGTERM) + wait loop sampai exit / timeout. Tidak pakai SIGKILL.
    """
    force_yes = "--yes" in argv
    wait_s = 20
    for a in argv[1:]:
        if isinstance(a, str) and a.startswith("--wait="):
            try:
                wait_s = max(1, int(a.split("=", 1)[1].strip()))
            except Exception:
                wait_s = 20
    ...
    ansi.print_system("CLOSED_APP (SOFT CLOSE)")
    print(f"{ansi.c_dim()}Profile:{ansi.c_reset()} {ansi.c_cyan()}{profile_obj.get('name','')}{ansi.c_reset()}")
    ...
    while True:
        still = [t for t in targets if _proc_running_exact(t)]
        if not still:
            ansi.print_system("✓ Semua target sudah exit (soft).")
            return 0
        if time.time() - t0 >= wait_s:
            print(f"\n{ansi.tag('WARN', ansi.c_yellow())} Timeout menunggu exit ({wait_s}s). Masih running:")
            for t in still:
                print(f"  - {ansi.c_cyan()}{t}{ansi.c_reset()}")
            print("Catatan: tidak pakai SIGKILL. Biasanya app nahan karena ada dialog/unsaved.")
            return 1
        time.sleep(0.4)
...
def _print_help() -> None:
    ...
    print("  ai run kill_all [name] : Soft-close apps dari profile (SIGTERM, tanpa paksa).")
    print("  ai run closed_app [name] [--wait=20] [--yes] : Soft-close + tunggu exit (tanpa paksa).")
    print("\nConfig Path:")
...
def handle(argv: list[str], cfg: dict) -> int:
    ...
    # session control
    if action == "kill_all":
        return cmd_kill_all(data, argv)
    if action == "closed_app":
        return cmd_closed_app(data, argv)
