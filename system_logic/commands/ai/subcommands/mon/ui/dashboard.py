"""
MON Live Dashboard.

CHANGELOG (Refactor):
- WiFiReader      : sekarang diimport dari collectors.network
- _read_power()   : delegasi ke PowerReader dari collectors.hardware
- _draw_ping_graph: diimport dari ui.render (hapus duplikat)
- NetSpeedometer  : alias ke render.NetSpeedTracker (hapus duplikat)
- SentinelEngine  : diinstansiasi di run_live_dashboard(), tick() dipanggil
                    setiap loop dengan ram_pct & swap_pct dari psutil
- GUARD line      : status sentinel ditampilkan di UI dashboard
"""

from __future__ import annotations

import os
import sys
import time
import shutil
import socket
import textwrap
import threading
import subprocess
from collections import deque
from typing import Optional, List, Tuple, Dict, Any

try:
    import psutil
except ImportError:
    psutil = None

from system_logic.terminal import ansi
from system_logic.core.paths import MON_HISTORY_DB, MON_HISTORY_LOCK
from system_logic.core.storage import snapshot_metric_sql, ensure_dir

from ..config import load_config
from ..sentinel import SentinelEngine
from ..collectors.system  import collect_system_stats
from ..collectors.hardware import collect_hardware_stats, collect_battery_stats, PowerReader
from ..collectors.network  import (
    collect_network_stats,
    collect_wifi_stats,
    WiFiReader,
    PingMonitor,
)
from ..collectors.process import collect_process_stats

from ..utils import (
    draw_bar,
    human_bytes,
    human_rate_bps,
    align_lr,
    now_ts,
    clamp,
    trim_name,
    trim_tail,
    col_by_temp,
    col_by_ping,
    col_by_dbm,
)

# Visual components — single source of truth
from .render import draw_ping_graph, NetSpeedTracker


# ==========================================================
# SYSTEM WRAPPERS
# ==========================================================

def get_cpu_percent() -> float:
    """Get CPU percentage."""
    return float(collect_system_stats().get("cpu_pct", 0.0))


def get_memory_info() -> Tuple[float, float, float]:
    """Get memory info: (pct, used_bytes, total_bytes)."""
    s = collect_system_stats()
    return float(s.get("ram_pct", 0.0)), float(s.get("ram_used", 0)), float(s.get("ram_total", 1))


def get_swap_info() -> Dict[str, Any]:
    """Get swap info."""
    s = collect_system_stats()
    return {
        "ok":    s.get("ok", False),
        "pct":   float(s.get("swap_pct",  0.0)),
        "used":  int(s.get("swap_used",   0)),
        "total": int(s.get("swap_total",  0)),
    }


def get_loadavg() -> Tuple[float, float, float]:
    """Get load average: (1m, 5m, 15m)."""
    s = collect_system_stats()
    return (
        float(s.get("load_1m",  0.0)),
        float(s.get("load_5m",  0.0)),
        float(s.get("load_15m", 0.0)),
    )


def get_uptime_str() -> str:
    """Get formatted uptime string."""
    secs = collect_system_stats().get("uptime_sec", 0)
    if not secs:
        return "?"
    d = int(secs // 86400); secs %= 86400
    h = int(secs // 3600)
    m = int((secs % 3600) // 60)
    return f"{d}d {h}h {m}m"


# ==========================================================
# HARDWARE WRAPPERS
# (Sebelumnya duplikat logic sysfs ada di sini;
#  sekarang cukup delegasi ke PowerReader dari collectors)
# ==========================================================

def read_temperatures() -> Dict[str, Optional[float]]:
    """Get key temperature readings (CPU, SSD, WiFi)."""
    return _read_key_temps()


def read_fan_rpm() -> Optional[int]:
    """Get fan RPM."""
    return _read_fan_rpm()


def read_power() -> Dict[str, Any]:
    """
    Get live power data via sysfs.
    Delegasi ke PowerReader dari collectors.hardware.
    """
    return PowerReader().read()


def read_power_capacity() -> Dict[str, Any]:
    """
    Get battery capacity/health.
    Delegasi ke PowerReader dari collectors.hardware.
    """
    pr  = PowerReader()
    cap = pr.read_capacity_health()
    if not cap.get("ok"):
        return {"ok": False}
    return {
        "ok":         True,
        "model":      cap.get("model", "BAT"),
        "health_pct": cap.get("health_pct", 0.0),
        "full":       cap.get("full",       0.0),
        "design":     cap.get("design",     0.0),
        "unit":       cap.get("unit",       "Wh"),
        "cycle":      cap.get("cycle",      "?"),
    }


# ==========================================================
# PROCESS WRAPPERS
# ==========================================================

def get_top_cpu_processes(total_cpu: float, n: int = 3) -> List[Tuple[str, float, float]]:
    """Get top N processes by CPU. Returns [(name, cpu_pct, share_pct)]."""
    procs = collect_process_stats(limit=20)
    if not procs.get("ok"):
        return []

    sorted_p = sorted(
        procs.get("processes", []),
        key=lambda x: x.get("cpu_pct", 0.0),
        reverse=True,
    )
    result = []
    for p in sorted_p[:n]:
        cpu_pct = float(p.get("cpu_pct", 0.0))
        share   = clamp((cpu_pct / total_cpu) * 100.0, 0.0, 100.0) if total_cpu > 0.1 else 0.0
        result.append((p.get("name", "?"), cpu_pct, share))
    return result


def get_top_mem_processes(n: int = 3) -> List[Tuple[str, float]]:
    """Get top N processes by RAM. Returns [(name, rss_gib)]."""
    procs = collect_process_stats(limit=n)
    if not procs.get("ok"):
        return []
    return [
        (p.get("name", "?"), float(p.get("rss_mb", 0.0)) / 1024.0)
        for p in procs.get("processes", [])
    ]


# ==========================================================
# NETWORK — NetSpeedometer (alias ke render.NetSpeedTracker)
# Nama lama dipertahankan untuk backwards compat
# ==========================================================

class NetSpeedometer(NetSpeedTracker):
    """
    Alias backwards-compatible ke render.NetSpeedTracker.
    Nama 'NetSpeedometer' dipertahankan agar tidak ada yang breaking.
    """
    pass


# ==========================================================
# PING SAMPLER (untuk live dashboard — pakai ping binary)
# Ini berbeda dari PingMonitor (TCP socket) di collectors/network.py
# Dipertahankan karena interface dashboard butuh .last_ms / .avg() / .jitter()
# ==========================================================

class PingSampler:
    """Background ping sampler menggunakan system ping binary."""

    def __init__(self, target: str, interval: float = 5.0) -> None:
        self.target   = target
        self.interval = float(clamp(interval, 1.0, 30.0))
        self.last_ms: Optional[float] = None
        self.window:  deque = deque(maxlen=20)
        self._stop    = threading.Event()
        self._thr     = threading.Thread(target=self._run, daemon=True)

    def start(self) -> None:
        if not self._thr.is_alive():
            self._thr.start()

    def stop(self) -> None:
        self._stop.set()

    def avg(self) -> Optional[float]:
        if not self.window:
            return None
        return sum(self.window) / float(len(self.window))

    def jitter(self) -> Optional[float]:
        if len(self.window) < 3:
            return None
        return max(self.window) - min(self.window)

    def _ping_once(self) -> Optional[float]:
        if not shutil.which("ping"):
            return None
        try:
            out = subprocess.check_output(
                ["ping", "-c", "1", "-W", "1", self.target],
                stderr=subprocess.DEVNULL,
                text=True,
                timeout=2,
            )
            if "time=" in out:
                return float(out.split("time=", 1)[1].split()[0])
        except Exception:
            pass
        return None

    def _run(self) -> None:
        while not self._stop.is_set():
            t0 = time.time()
            ms = self._ping_once()
            if ms is not None:
                self.last_ms = ms
                self.window.append(ms)
            else:
                self.last_ms = None
            wait = self.interval - (time.time() - t0)
            if wait > 0:
                self._stop.wait(wait)


# ==========================================================
# DISK I/O SAMPLER
# ==========================================================

class DiskIOSampler:
    """Sampling /sys/block/<dev>/stat untuk active% dan R/W speed."""

    def __init__(self, dev: str) -> None:
        self.dev         = dev
        self.t0          = time.time()
        self.prev        = self._read_stat()
        self.sector_size = self._read_sector_size()
        self.active_pct  = 0.0
        self.read_bps    = 0.0
        self.write_bps   = 0.0

    def _read_sector_size(self) -> int:
        from pathlib import Path
        p = Path(f"/sys/block/{self.dev}/queue/hw_sector_size")
        try:
            v = int(p.read_text().strip())
            return v if v > 0 else 512
        except Exception:
            return 512

    def _read_stat(self) -> Optional[List[int]]:
        from pathlib import Path
        p = Path(f"/sys/block/{self.dev}/stat")
        try:
            return [int(x) for x in p.read_text().strip().split()]
        except Exception:
            return None

    def update(self) -> None:
        cur = self._read_stat()
        t1  = time.time()
        dt  = t1 - self.t0

        if cur is None or self.prev is None or dt <= 0:
            self.t0 = t1; self.prev = cur
            self.active_pct = self.read_bps = self.write_bps = 0.0
            return

        try:
            d_read  = cur[2] - self.prev[2]
            d_write = cur[6] - self.prev[6]
            d_ioms  = cur[9] - self.prev[9]
            self.read_bps   = (d_read  * self.sector_size) / dt
            self.write_bps  = (d_write * self.sector_size) / dt
            self.active_pct = clamp((d_ioms / (dt * 1000.0)) * 100.0, 0.0, 100.0)
        except Exception:
            self.active_pct = self.read_bps = self.write_bps = 0.0

        self.t0 = t1; self.prev = cur


# ==========================================================
# INTERNAL HELPERS (direct psutil / sysfs reads)
# ==========================================================

def _swap_read() -> Dict[str, Any]:
    """Baca swap memory via psutil."""
    out: Dict[str, Any] = {"ok": False, "pct": 0.0, "used": 0, "total": 0}
    if psutil is None:
        return out
    try:
        sw = psutil.swap_memory()
        out.update({"ok": True, "pct": float(sw.percent), "used": int(sw.used), "total": int(sw.total)})
    except Exception:
        pass
    return out


def _read_key_temps() -> Dict[str, Optional[float]]:
    """Baca temperatur kunci (CPU/NVMe/WiFi) via psutil."""
    res: Dict[str, Optional[float]] = {"cpu": None, "ssd": None, "wifi": None}
    if psutil is None:
        return res
    try:
        temps = psutil.sensors_temperatures(fahrenheit=False) or {}
    except Exception:
        return res

    for key in ("coretemp", "k10temp"):
        entries = temps.get(key) or []
        best = None
        for e in entries:
            lab = (getattr(e, "label", "") or "").lower()
            if "package" in lab or "tctl" in lab:
                best = getattr(e, "current", None); break
        if best is None and entries:
            best = getattr(entries[0], "current", None)
        if best is not None:
            res["cpu"] = float(best); break

    for k, entries in temps.items():
        lk = k.lower()
        if "nvme" in lk or "composite" in lk:
            for e in entries:
                lab = (getattr(e, "label", "") or "").lower()
                if "composite" in lab or lab == "":
                    cur = getattr(e, "current", None)
                    if cur is not None:
                        res["ssd"] = float(cur); break
            if res["ssd"] is not None:
                break

    for k, entries in temps.items():
        lk = k.lower()
        if "iwl" in lk or "wifi" in lk or "ath" in lk:
            cur = getattr(entries[0], "current", None) if entries else None
            if cur is not None:
                res["wifi"] = float(cur); break

    return res


def _read_fan_rpm() -> Optional[int]:
    """Baca fan RPM via psutil."""
    if psutil is None:
        return None
    try:
        fans = psutil.sensors_fans() or {}
    except Exception:
        return None
    best: Optional[int] = None
    for _k, entries in fans.items():
        for e in entries:
            lab = (getattr(e, "label", "") or "").lower()
            cur = getattr(e, "current", None)
            if cur is None: continue
            if "cpu" in lab: return int(cur)
            if best is None or int(cur) > best: best = int(cur)
    return best


def _read_power() -> Dict[str, Any]:
    """
    Baca live power. Delegasi ke PowerReader.
    Wrapper ini dipertahankan agar kode render loop tidak perlu diubah.
    """
    return PowerReader().read()


# ==========================================================
# LAYOUT HELPERS
# ==========================================================

_IGNORED_FS_TYPES = {
    "proc", "sysfs", "devtmpfs", "devpts", "tmpfs", "cgroup", "cgroup2",
    "pstore", "securityfs", "debugfs", "tracefs", "configfs", "efivarfs",
    "mqueue", "hugetlbfs", "fusectl", "autofs", "overlay", "squashfs",
}


def _mounted_partitions() -> List[Dict[str, str]]:
    """Return mounted partitions (non-virtual)."""
    if psutil is None:
        return []
    items: List[Dict[str, str]] = []
    try:
        parts = psutil.disk_partitions(all=False)
    except Exception:
        return []

    for p in parts:
        mp     = (getattr(p, "mountpoint", "") or "").strip()
        dev    = (getattr(p, "device",     "") or "").strip()
        fstype = (getattr(p, "fstype",     "") or "").strip().lower()

        if not mp or not dev: continue
        if fstype in _IGNORED_FS_TYPES: continue
        if mp.startswith(("/proc", "/sys", "/dev")): continue
        if mp.startswith("/run") and not mp.startswith("/run/media"): continue
        bdev = os.path.basename(dev)
        if bdev.startswith(("loop", "zram", "ram", "sr")): continue

        items.append({"mount": mp, "device": dev, "fstype": fstype})

    def _key(x: Dict[str, str]) -> Tuple[int, str]:
        mp = x.get("mount", "")
        if mp == "/":     return (0, mp)
        if mp == "/home": return (1, mp)
        return (2, mp)

    return sorted(items, key=_key)


def _default_disk_label(mountpoint: str, device: str) -> str:
    if mountpoint == "/":     return "root"
    if mountpoint == "/home": return "home"
    base = os.path.basename(mountpoint.rstrip("/"))
    return base if base else (os.path.basename(device) or "disk")


def _pick_root_disk_dev() -> Optional[str]:
    """Best-effort: choose disk backing '/'."""
    from pathlib import Path
    try:
        p = subprocess.run(
            ["findmnt", "-n", "-o", "SOURCE", "/"],
            capture_output=True, text=True, timeout=2,
        )
        if p.returncode == 0 and p.stdout:
            src = p.stdout.strip()
            if src.startswith("/dev/"):
                base = os.path.basename(src)
                p2   = subprocess.run(
                    ["lsblk", "-no", "PKNAME", f"/dev/{base}"],
                    capture_output=True, text=True, timeout=2,
                )
                if p2.returncode == 0 and p2.stdout.strip():
                    return p2.stdout.strip()
                if Path(f"/sys/block/{base}").exists():
                    return base
    except Exception:
        pass
    if Path("/sys/block/nvme0n1").exists():
        return "nvme0n1"
    if shutil.which("lsblk"):
        try:
            p = subprocess.run(
                ["lsblk", "-d", "-n", "-o", "NAME,TYPE"],
                capture_output=True, text=True, timeout=2,
            )
            if p.returncode == 0 and p.stdout:
                for ln in p.stdout.splitlines():
                    parts = ln.split()
                    if len(parts) >= 2 and parts[1] == "disk":
                        return parts[0].strip()
        except Exception:
            pass
    return None


def _net_quality(
    ping_avg_ms: Optional[float],
    jitter_ms:   Optional[float],
    wifi_dbm:    Optional[float],
) -> Tuple[str, str]:
    """Return (label, color) based on network metrics."""
    score = 0
    if isinstance(ping_avg_ms, (int, float)):
        score += 2 if ping_avg_ms <= 40 else (1 if ping_avg_ms <= 120 else 0)
    if isinstance(jitter_ms, (int, float)):
        score += 2 if jitter_ms <= 15 else (1 if jitter_ms <= 40 else 0)
    if isinstance(wifi_dbm, (int, float)):
        score += 2 if wifi_dbm >= -60 else (1 if wifi_dbm >= -70 else 0)
    else:
        score += 1

    if score >= 5: return "FAST", ansi.c_green()
    if score >= 3: return "OK",   ansi.c_yellow()
    return "SLOW", ansi.c_red()


def _fan_label(rpm: Optional[int]) -> str:
    if rpm is None or not isinstance(rpm, int) or rpm < 0:
        return f"{ansi.c_dim()}N/A{ansi.c_reset()}"
    if rpm == 0:
        return f"{ansi.c_dim()}0 RPM (OFF){ansi.c_reset()}"
    if rpm <= 1000: lvl = f"{ansi.c_green()}Very Low{ansi.c_reset()}"
    elif rpm <= 3000: lvl = f"{ansi.c_green()}Low{ansi.c_reset()}"
    elif rpm <= 4500: lvl = f"{ansi.c_yellow()}Medium{ansi.c_reset()}"
    else: lvl = f"{ansi.c_red()}High{ansi.c_reset()}"
    return f"{lvl}{ansi.c_dim()} ({rpm} RPM){ansi.c_reset()}"


def _distro_pretty() -> str:
    from pathlib import Path
    p = Path("/etc/os-release")
    if not p.exists(): return ""
    try:
        for ln in p.read_text().splitlines():
            if ln.startswith("PRETTY_NAME="):
                return ln.split("=", 1)[1].strip().strip('"')
    except Exception:
        pass
    return ""


def _uname_kernel() -> str:
    try:
        return os.uname().release
    except Exception:
        from pathlib import Path
        try: return Path("/proc/sys/kernel/osrelease").read_text().strip()
        except Exception: return "-"


def _cpu_model() -> str:
    from pathlib import Path
    try:
        for ln in Path("/proc/cpuinfo").read_text().splitlines():
            if ln.lower().startswith("model name"):
                return ln.split(":", 1)[-1].strip()
    except Exception:
        pass
    return ""


def _nvme_model_hint(dev: Optional[str]) -> str:
    if not dev: return ""
    from pathlib import Path
    sys_p = Path(f"/sys/block/{dev}/device/model")
    if sys_p.exists():
        try: return sys_p.read_text().strip()
        except Exception: pass
    if shutil.which("lsblk"):
        try:
            p = subprocess.run(
                ["lsblk", "-d", "-n", "-o", "MODEL", f"/dev/{dev}"],
                capture_output=True, text=True, timeout=2,
            )
            if p.returncode == 0 and p.stdout:
                return p.stdout.splitlines()[0].strip()
        except Exception:
            pass
    return ""


def _cpu_freq_info() -> Dict[str, Any]:
    from pathlib import Path
    gov = None
    gov_p = Path("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor")
    if gov_p.exists():
        try: gov = gov_p.read_text().strip() or None
        except Exception: pass

    vals: List[float] = []
    try:
        for p in sorted(Path("/sys/devices/system/cpu").glob("cpu[0-9]*/cpufreq/scaling_cur_freq")):
            try:
                v = int(p.read_text().strip())
                if v > 0: vals.append(float(v) / 1e6)
            except Exception:
                continue
    except Exception:
        pass

    if vals:
        return {"ok": True, "ghz": sum(vals) / len(vals), "gov": gov}

    if psutil is not None:
        try:
            fr = psutil.cpu_freq()
            if fr and getattr(fr, "current", None):
                return {"ok": True, "ghz": float(fr.current) / 1000.0, "gov": gov}
        except Exception:
            pass

    return {"ok": False, "ghz": None, "gov": gov}


def _fmt_topcpu(rows: List[Tuple[str, float, float]], name_w: int = 16) -> List[str]:
    out: List[str] = []
    for i, (nm, pct, share) in enumerate(rows, start=1):
        nm2 = trim_name(nm, name_w)
        if i == 1:
            out.append(
                f"  {i}) {nm2:<{name_w}} {pct:>5.1f}%   "
                f"{ansi.c_dim()}(≈{share:.0f}% of total){ansi.c_reset()}"
            )
        else:
            out.append(f"  {i}) {nm2:<{name_w}} {pct:>5.1f}%")
    return out


def _fmt_topmem(rows: List[Tuple[str, float]], name_w: int = 16) -> List[str]:
    return [
        f"  {i}) {trim_name(nm, name_w):<{name_w}} {gib:>5.2f} GiB"
        for i, (nm, gib) in enumerate(rows, start=1)
    ]


# ==========================================================
# THROTTLE READER
# ==========================================================

class ThrottleReader:
    """Best-effort thermal throttling indicator (Intel-friendly)."""

    def __init__(self) -> None:
        self.prev_core: Optional[int] = None
        self.prev_pkg:  Optional[int] = None

    def read(self) -> Dict[str, Any]:
        from pathlib import Path
        base   = Path("/sys/devices/system/cpu/cpu0/thermal_throttle")
        core_p = base / "core_throttle_count"
        pkg_p  = base / "package_throttle_count"

        if not core_p.exists() and not pkg_p.exists():
            return {"ok": False}

        def _ri(p: Any) -> Optional[int]:
            try: return int(p.read_text().strip())
            except Exception: return None

        core = _ri(core_p) if core_p.exists() else None
        pkg  = _ri(pkg_p)  if pkg_p.exists()  else None

        delta_core = (core - self.prev_core) if isinstance(core, int) and isinstance(self.prev_core, int) else None
        delta_pkg  = (pkg  - self.prev_pkg)  if isinstance(pkg,  int) and isinstance(self.prev_pkg,  int) else None

        if isinstance(core, int): self.prev_core = core
        if isinstance(pkg,  int): self.prev_pkg  = pkg

        active = (isinstance(delta_core, int) and delta_core > 0) or \
                 (isinstance(delta_pkg,  int) and delta_pkg  > 0)

        return {
            "ok": True, "core": core, "pkg": pkg,
            "delta_core": delta_core, "delta_pkg": delta_pkg, "active": active,
        }


# ==========================================================
# BOX LINE HELPER
# ==========================================================

def _box_line(left: str, right: str, width: int, vert: str) -> str:
    """Create a box line with left and right content."""
    import re
    ansi_re = re.compile(r"\x1b\[[0-9;]*m")
    lv      = len(ansi_re.sub("", left))
    rv      = len(ansi_re.sub("", right))
    padding = max(0, width - 4 - lv - rv)
    return f"{vert} {left}{' ' * padding}{right} {vert}"


# ==========================================================
# MAIN DASHBOARD RUNNER
# ==========================================================

def run_live_dashboard(argv: List[str]) -> int:
    """
    Live HUD.

    Options:
      --interval N
      --compact
      --target HOST
      --no-disks
      --maxwidth N
    """
    if psutil is None:
        ansi.print_brief_error("Monitoring requires 'psutil' library.")
        print("Install (Fedora): sudo dnf install python3-psutil")
        return 1

    interval   = 1.0
    compact    = False
    target     = "google.com"
    show_disks = True
    maxwidth:  Optional[int] = None

    i = 0
    while i < len(argv):
        a = argv[i]
        if a in ("--compact", "compact"):
            compact = True
        elif a == "--no-disks":
            show_disks = False
        elif a == "--interval" and i + 1 < len(argv):
            try: interval = float(argv[i + 1])
            except Exception: pass
            i += 1
        elif a == "--target" and i + 1 < len(argv):
            target = argv[i + 1].strip() or target
            i += 1
        elif a == "--maxwidth" and i + 1 < len(argv):
            try: maxwidth = int(float(argv[i + 1]))
            except Exception: maxwidth = None
            i += 1
        i += 1

    interval = float(clamp(interval, 0.2, 5.0))

    # Initialize
    ns       = NetSpeedometer()
    ns.update()
    disk_dev = _pick_root_disk_dev()
    disk_io  = DiskIOSampler(disk_dev) if disk_dev else None
    ping     = PingSampler(target=target, interval=max(2.0, interval * 2.0))
    thr      = ThrottleReader()
    ping.start()

    # Sentinel — jalan bersama dashboard, tidur sampai ada yang perlu dikhawatirkan
    sentinel = SentinelEngine()
    sentinel_result: Dict[str, Any] = {
        "active":      sentinel.active,
        "alert_level": 0,
        "status":      "SLEEP",
        "phase":       "SLEEP",
    }

    # Throttle timers
    last_snap = 0.0; SNAP_EVERY = 600.0
    last_hw   = 0.0; HW_EVERY   =  30.0
    last_top  = 0.0; TOP_EVERY  =   5.0
    last_disks= 0.0; DISK_EVERY =  10.0
    last_freq = 0.0; FREQ_EVERY =   2.0

    hw_cpu = hw_nvme = ""
    distro = _distro_pretty()
    kernel = _uname_kernel()
    host   = socket.gethostname()

    top_cpu_rows: List[str] = []
    top_mem_rows: List[str] = []
    disk_cache:   List[Dict[str, Any]] = []
    cpu_freq_str  = f"{ansi.c_dim()}N/A{ansi.c_reset()}"

    input_muter = ansi.MuteInputDuringWait()
    ansi.alt_screen_enter()
    ansi.cursor_hide()

    try:
        # Prime CPU percent
        try: psutil.cpu_percent(interval=None)
        except Exception: pass
        try:
            for p in psutil.process_iter():
                try: p.cpu_percent(interval=None)
                except Exception: continue
        except Exception:
            pass

        with input_muter:
            while True:
                cols, _rows = ansi.term_size()
                usable = min(cols, 96)
                if isinstance(maxwidth, int) and maxwidth > 40:
                    usable = min(usable, int(maxwidth))

                # Core metrics
                try: cpu = float(psutil.cpu_percent(interval=None))
                except Exception: cpu = 0.0

                try:
                    vm = psutil.virtual_memory()
                    ram_pct   = float(vm.percent)
                    ram_used  = float(vm.used)
                    ram_total = float(vm.total)
                except Exception:
                    ram_pct = ram_used = ram_total = 0.0

                swap    = _swap_read()
                temps   = _read_key_temps()
                fan_rpm = _read_fan_rpm()
                power   = _read_power()          # → PowerReader().read()
                wifi    = WiFiReader.read()       # → collect_wifi_stats()

                # Sentinel tick — kirim data dari loop, terima status
                swap_pct_f = float(swap.get("pct", 0.0))
                sentinel_result = sentinel.tick(ram_pct, swap_pct_f)

                ns.update()
                if disk_io:
                    disk_io.update()

                # Throttled freq
                if time.time() - last_freq >= FREQ_EVERY:
                    fi  = _cpu_freq_info()
                    gov = fi.get("gov") or ""
                    gov_txt = f"{ansi.c_dim()}({gov}){ansi.c_reset()}" if gov else ""
                    if fi.get("ok") and isinstance(fi.get("ghz"), (int, float)):
                        cpu_freq_str = f"{float(fi['ghz']):.2f} GHz {gov_txt}".rstrip()
                    else:
                        cpu_freq_str = f"{ansi.c_dim()}N/A{ansi.c_reset()} {gov_txt}".rstrip()
                    last_freq = time.time()

                # Throttled top processes
                if time.time() - last_top >= TOP_EVERY:
                    t_cpu = get_top_cpu_processes(cpu, n=3)
                    t_mem = get_top_mem_processes(n=3)
                    top_cpu_rows = _fmt_topcpu(t_cpu) if t_cpu else [f"  {ansi.c_dim()}(no data){ansi.c_reset()}"]
                    top_mem_rows = _fmt_topmem(t_mem) if t_mem else [f"  {ansi.c_dim()}(no data){ansi.c_reset()}"]
                    last_top = time.time()

                # Throttled disks
                if show_disks and (time.time() - last_disks >= DISK_EVERY):
                    mounts     = _mounted_partitions()
                    disk_cache = []
                    for m in mounts:
                        mp    = m["mount"]
                        dev   = m["device"]
                        label = _default_disk_label(mp, dev)
                        try:
                            du       = shutil.disk_usage(mp)
                            used_pct = (du.used / du.total) * 100.0 if du.total else 0.0
                            disk_cache.append({
                                "label": label, "mount": mp, "device": dev,
                                "used": du.used, "total": du.total,
                                "free": du.free, "pct": used_pct,
                            })
                        except Exception:
                            continue
                    last_disks = time.time()

                # Throttled hardware info
                if time.time() - last_hw >= HW_EVERY:
                    hw_cpu  = _cpu_model()
                    hw_nvme = _nvme_model_hint(disk_dev)
                    last_hw = time.time()

                # Battery status
                lvl     = power.get("percent")
                lvl_i   = int(lvl) if isinstance(lvl, int) else None
                st_raw  = str(power.get("status_raw") or "Unknown")
                plugged = power.get("plugged")
                plugged_b = bool(plugged) if isinstance(plugged, bool) else False

                bat_col = ansi.c_green()
                if plugged_b:
                    if st_raw.lower() == "full" or lvl_i == 100:
                        st_txt = "AC (Full)";            bat_col = ansi.c_cyan()
                    elif st_raw.lower() == "not charging":
                        st_txt = "AC (Smart Cut-off)";   bat_col = ansi.c_cyan()
                    elif "charg" in st_raw.lower():
                        st_txt = "AC (Charging)"
                    else:
                        st_txt = f"AC ({st_raw})"
                else:
                    st_txt = "DC (On Battery)"
                    if lvl_i is not None and lvl_i < 20:   bat_col = ansi.c_red()
                    elif lvl_i is not None and lvl_i < 35: bat_col = ansi.c_yellow()

                secs_left = power.get("secs_left")
                eta = ""
                if isinstance(secs_left, (int, float)) and 0 < secs_left < 10**9:
                    eta = f"{int(secs_left//3600)}h {int((secs_left%3600)//60)}m left"

                # Periodic battery snapshot (SQLite)
                if time.time() - last_snap >= SNAP_EVERY:
                    cap = read_power_capacity()
                    if cap.get("ok"):
                        model  = str(cap.get("model") or "BAT")
                        health = round(float(cap["health_pct"]), 2)
                        full   = round(float(cap["full"]), 2)
                        ensure_dir(MON_HISTORY_DB.parent)
                        snapshot_metric_sql(MON_HISTORY_DB, MON_HISTORY_LOCK, "battery", model, "health_pct",    health)
                        snapshot_metric_sql(MON_HISTORY_DB, MON_HISTORY_LOCK, "battery", model, "capacity_full", full)
                    last_snap = time.time()

                # === RENDER ===
                _out: List[str] = []
                _ap = _out.append
                sys.stdout.write("\x1b[H\x1b[J")

                # Header
                ts = now_ts()
                _ap(align_lr(
                    f"{ansi.c_cyan()}{ansi.c_bold()}AI MON{ansi.c_reset()} {ansi.c_dim()}• Live Cockpit{ansi.c_reset()}",
                    f"{ansi.c_dim()}{ts}{ansi.c_reset()}",
                    usable,
                ))

                sep = f"{ansi.c_dim()}{ansi.hr(min(usable, 96))}{ansi.c_reset()}"

                _ap(align_lr(
                    f"{ansi.c_dim()}Host:{ansi.c_reset()} {host}  {ansi.c_dim()}OS:{ansi.c_reset()} {distro or '-'}",
                    f"{ansi.c_dim()}Kernel:{ansi.c_reset()} {kernel}",
                    usable,
                ))

                cpu_count = os.cpu_count() or 1
                _ap(align_lr(
                    f"{ansi.c_dim()}Uptime:{ansi.c_reset()} {get_uptime_str()}  {ansi.c_dim()}CPUs:{ansi.c_reset()} {cpu_count}",
                    f"{ansi.c_dim()}Interval:{ansi.c_reset()} {interval:.1f}s  {ansi.c_dim()}Target:{ansi.c_reset()} {target}",
                    usable,
                ))

                la1, la5, la15 = get_loadavg()
                per1 = (la1 / cpu_count) if cpu_count else 0.0
                _ap(align_lr(
                    f"{ansi.c_dim()}Load avg (1m/5m/15m):{ansi.c_reset()} {la1:.2f}/{la5:.2f}/{la15:.2f}",
                    f"{ansi.c_dim()}Load/CPU:{ansi.c_reset()} {per1:.2f}/{(la5/cpu_count if cpu_count else 0):.2f}/{(la15/cpu_count if cpu_count else 0):.2f}",
                    usable,
                ))

                _ap(sep)

                _ap(f"{ansi.c_bold()} CPU {ansi.c_reset()} {draw_bar(cpu)} {cpu:>5.1f}%   {ansi.c_dim()}Freq:{ansi.c_reset()} {cpu_freq_str}")
                _ap(f"{ansi.c_bold()} RAM {ansi.c_reset()} {draw_bar(ram_pct)} {ram_pct:>5.1f}%   {ram_used/(1024**3):.1f}/{ram_total/(1024**3):.1f} GiB")

                if swap.get("ok") and swap.get("total", 0) > 0:
                    sw_pct = float(swap.get("pct", 0.0))
                    _ap(f"{ansi.c_bold()}SWAP {ansi.c_reset()} {draw_bar(sw_pct)} {sw_pct:>5.1f}%   "
                        f"{human_bytes(int(swap.get('used',0)))}/{human_bytes(int(swap.get('total',0)))}")
                else:
                    _ap(f"{ansi.c_bold()}SWAP {ansi.c_reset()} {ansi.c_dim()}(not available){ansi.c_reset()}")

                if disk_io and disk_dev:
                    util = float(disk_io.active_pct)
                    col_u = ansi.c_green() if util <= 60 else (ansi.c_yellow() if util <= 85 else ansi.c_red())
                    _ap(f"{ansi.c_bold()} I/O {ansi.c_reset()} {col_u}{util:>5.1f}%{ansi.c_reset()}  "
                        f"R:{human_rate_bps(disk_io.read_bps):<11}  "
                        f"W:{human_rate_bps(disk_io.write_bps):<11}  "
                        f"{ansi.c_dim()}({disk_dev}){ansi.c_reset()}")

                _ap(sep)

                _ap(f"{ansi.c_bold()}Top CPU (1–3){ansi.c_reset()}")
                for ln in top_cpu_rows: _ap(ln)
                _ap("")
                _ap(f"{ansi.c_bold()}Top MEM (1–3){ansi.c_reset()}")
                for ln in top_mem_rows: _ap(ln)

                _ap(sep)

                cpu_t  = temps.get("cpu")
                ssd_t  = temps.get("ssd")
                wifi_t = temps.get("wifi")
                _ap(
                    f"{ansi.c_bold()}TEMP {ansi.c_reset()}"
                    f"CPU:{col_by_temp(cpu_t)}{f'{cpu_t:.1f}°C' if isinstance(cpu_t,(int,float)) else '-'}{ansi.c_reset()}   "
                    f"NVMe:{col_by_temp(ssd_t)}{f'{ssd_t:.1f}°C' if isinstance(ssd_t,(int,float)) else '-'}{ansi.c_reset()}   "
                    f"WiFi:{col_by_temp(wifi_t)}{f'{wifi_t:.1f}°C' if isinstance(wifi_t,(int,float)) else '-'}{ansi.c_reset()}   "
                    f"{ansi.c_dim()}Fan:{ansi.c_reset()} "
                    f"{_fan_label(int(fan_rpm) if isinstance(fan_rpm,(int,float)) else None)}"
                )

                th = thr.read()
                if th.get("ok"):
                    if th.get("active"):
                        parts = []
                        dc, dp = th.get("delta_core"), th.get("delta_pkg")
                        if isinstance(dc, int) and dc > 0: parts.append(f"+core {dc}")
                        if isinstance(dp, int) and dp > 0: parts.append(f"+pkg {dp}")
                        extra = f" {ansi.c_dim()}({', '.join(parts)}){ansi.c_reset()}" if parts else ""
                        _ap(f"{ansi.c_bold()}THROT{ansi.c_reset()} {ansi.c_red()}ACTIVE{ansi.c_reset()}{extra}")
                    else:
                        _ap(f"{ansi.c_bold()}THROT{ansi.c_reset()} {ansi.c_green()}OK{ansi.c_reset()}")
                else:
                    _ap(f"{ansi.c_bold()}THROT{ansi.c_reset()} {ansi.c_dim()}N/A{ansi.c_reset()}")

                # GUARD — Sentinel status
                s_level  = sentinel_result.get("alert_level", 0)
                s_status = sentinel_result.get("status", "SLEEP")
                s_phase  = sentinel_result.get("phase",  "SLEEP")
                s_active = sentinel_result.get("active", False)

                if not s_active:
                    guard_line = f"{ansi.c_bold()}GUARD{ansi.c_reset()} {ansi.c_dim()}DISABLED{ansi.c_reset()}"
                elif s_level == 0:
                    guard_line = (
                        f"{ansi.c_bold()}GUARD{ansi.c_reset()} "
                        f"{ansi.c_dim()}SLEEP{ansi.c_reset()} "
                        f"{ansi.c_dim()}— sistem aman, sentinel standby{ansi.c_reset()}"
                    )
                elif s_level == 1:
                    guard_line = (
                        f"{ansi.c_bold()}GUARD{ansi.c_reset()} "
                        f"{ansi.c_cyan()}{s_phase}{ansi.c_reset()} "
                        f"{ansi.c_dim()}— {s_status}{ansi.c_reset()}"
                    )
                elif s_level == 2:
                    guard_line = (
                        f"{ansi.c_bold()}GUARD{ansi.c_reset()} "
                        f"{ansi.c_yellow()}{s_phase}{ansi.c_reset()} "
                        f"{ansi.c_yellow()}{s_status}{ansi.c_reset()}"
                    )
                else:
                    guard_line = (
                        f"{ansi.c_bold()}GUARD{ansi.c_reset()} "
                        f"{ansi.c_red()}{ansi.c_bold()}{s_phase}{ansi.c_reset()} "
                        f"{ansi.c_red()}{s_status}{ansi.c_reset()}"
                    )
                _ap(guard_line)

                lvl_txt = f"{lvl_i}%" if isinstance(lvl_i, int) else "?"
                _ap(
                    f"{ansi.c_bold()}POWER{ansi.c_reset()} "
                    f"{bat_col}{lvl_txt}{ansi.c_reset()} [{st_txt}]   "
                    f"{ansi.c_yellow()}{power.get('watt_str','N/A')}{ansi.c_reset()} "
                    f"@ {power.get('volt_str','N/A')} | {power.get('amp_str','N/A')}   "
                    f"{ansi.c_dim()}{eta}{ansi.c_reset()}"
                )

                _ap(sep)

                # Network
                rx      = human_rate_bps(ns.rx)
                tx      = human_rate_bps(ns.tx)
                ping_ms = ping.last_ms
                ping_avg= ping.avg()
                jit     = ping.jitter()

                ping_txt = "TO" if ping_ms is None else f"{ping_ms:.0f}ms"
                avg_txt  = "-"  if ping_avg is None else f"{ping_avg:.0f}ms"
                jit_txt  = "-"  if jit      is None else f"{jit:.0f}ms"
                pcol     = col_by_ping(ping_ms)

                wifi_sig = wifi.get("signal_dbm") if wifi.get("ok") else None
                q_lbl, q_col = _net_quality(ping_avg, jit, wifi_sig if isinstance(wifi_sig,(int,float)) else None)

                _ap(
                    f"{ansi.c_bold()}NET  {ansi.c_reset()}↓{rx}  ↑{tx}   "
                    f"Quality:{q_col}{q_lbl}{ansi.c_reset()}"
                )

                ping_line = (
                    f"{ansi.c_bold()}PING {ansi.c_reset()}{target}   "
                    f"last:{pcol}{ping_txt}{ansi.c_reset()}  "
                    f"avg:{ansi.c_dim()}{avg_txt}{ansi.c_reset()}  "
                    f"jitter:{ansi.c_dim()}{jit_txt}{ansi.c_reset()}"
                )
                if wifi.get("ok"):
                    ssid    = wifi.get("ssid") or "-"
                    sig     = wifi.get("signal_dbm")
                    sig_txt = f"{sig:.0f} dBm" if isinstance(sig,(int,float)) else "-"
                    ping_line += (
                        f"   {ansi.c_dim()}WiFi:{ansi.c_reset()} "
                        f"{trim_name(ssid,14)} ({col_by_dbm(sig)}{sig_txt}{ansi.c_reset()})"
                    )
                _ap(ping_line)

                # Disks
                if show_disks:
                    _ap(sep)
                    _ap(f"{ansi.c_bold()}DISKS{ansi.c_reset()} {ansi.c_dim()}(mounted partitions){ansi.c_reset()}")

                    if not disk_cache:
                        _ap(f"  {ansi.c_dim()}(no mounted disks found){ansi.c_reset()}")
                    else:
                        name_w = 6; free_w = 9; bar_w = 10; used_w = 15
                        fixed  = 2 + name_w + 3 + free_w + 3 + (bar_w + 5) + 3 + used_w + 3 + 7
                        mount_w = max(12, min(usable - fixed, 42))

                        if usable >= 70:
                            _ap(
                                f"  {ansi.c_dim()}"
                                f"{'NAME':<{name_w}} | {'FREE':<{free_w}} | "
                                f"{'USED%':<{bar_w+5}} | {'USED/TOTAL':<{used_w}} | MOUNT"
                                f"{ansi.c_reset()}"
                            )

                        for d in disk_cache:
                            pct   = float(d.get("pct",   0.0))
                            used  = float(d.get("used",  0.0))
                            total = float(d.get("total", 0.0))
                            free  = float(d.get("free",  0.0))
                            label = str(d.get("label") or "disk")
                            mp    = str(d.get("mount") or "-")
                            _ap(
                                f"  {ansi.c_bold()}{trim_name(label,name_w):<{name_w}}{ansi.c_reset()} | "
                                f"{human_bytes(free):<{free_w}} | "
                                f"{draw_bar(pct,width=bar_w)} {pct:>3.0f}%  | "
                                f"{human_bytes(used)}/{human_bytes(total):<{used_w}} | "
                                f"({trim_tail(mp,mount_w)})"
                            )

                # Footer
                if not compact:
                    hw_parts = []
                    if hw_cpu:  hw_parts.append(f"CPU: {hw_cpu.strip()}")
                    if hw_nvme: hw_parts.append(f"NVMe: {hw_nvme.strip()}")
                    if hw_parts:
                        for ln in textwrap.wrap(" | ".join(hw_parts), width=min(usable, 92)):
                            _ap(f"{ansi.c_dim()}{ln}{ansi.c_reset()}")

                _ap("")
                if compact:
                    _ap(f"{ansi.c_dim()}Ctrl+C untuk keluar. Tip: ai mon sensors{ansi.c_reset()}")
                else:
                    _ap(f"{ansi.c_dim()}Ctrl+C untuk keluar.{ansi.c_reset()}  "
                        f"{ansi.c_dim()}Tip:{ansi.c_reset()} `ai mon sensors` untuk daftar sensor lengkap.")
                    _ap(f"{ansi.c_dim()}Tip:{ansi.c_reset()} `sudo ai mon disk` untuk TBW/SMART detail.")
                    _ap(f"{ansi.c_dim()}Tip:{ansi.c_reset()} `ai mon help` untuk opsi `--disk` dan tuning layout.")

                sys.stdout.write("\n".join(_out) + "\n")
                sys.stdout.flush()

                time.sleep(interval)

    except KeyboardInterrupt:
        pass
    except Exception as ex:
        try: ansi.cursor_show(); ansi.alt_screen_exit()
        except Exception: pass
        ansi.print_brief_error(f"Dashboard crash: {type(ex).__name__}")
        print(str(ex))
        return 1
    finally:
        try: ping.stop()
        except Exception: pass
        try: ansi.cursor_show(); ansi.alt_screen_exit()
        except Exception: pass

    return 0