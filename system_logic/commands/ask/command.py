...
def _maybe_compact_api_oneshot(cfg: dict, original_user: str, answer: str) -> str:
    # Optional: ringkas jawaban API biar enak dibaca di terminal
    if not answer:
        return answer
...
def handle(argv: list[str], cfg: dict) -> int:
    ...
    # Optional: magic hello -> chat (legacy behavior, but opt-in)
    if (
        text.lower() in ("hello", "halo")
    ...
    if backend == "api":
        ...
        if not ok:
            ...
            try:
                ...
                flush_stdin()
                if mode in ("auto", "api") and not plain:
                    print_system("😿 API error. Aku pindah ke LOCAL dulu ya.")
                backend = "local"

    # LOCAL fallback / normal
    try:
        ...
        flush_stdin()
        _render_oneshot(cfg, user_text=text, backend="local", answer=_force_single_paragraph(ans), plain=plain)
        return 0
    except KeyboardInterrupt:
    ...
    try:
        ...
        while True:
            ...
            if backend == "api":
                ...
                if not ok:
                    ...
                    try:
                        ...
                        ask_ui.print_chat_turn(label, backend="api", answer=answer, plain=plain)
                        continue
                    except KeyboardInterrupt:
            ...
            try:
                ...
                flush_stdin()
                history.append({"role": "assistant", "content": answer})
                ask_ui.print_chat_turn(label, backend="local", answer=answer, plain=plain)
            except KeyboardInterrupt:
        ...
        # Optional memory summary (API only)
